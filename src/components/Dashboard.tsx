...
const OVERLAY_FEATURE_MAP: Partial<Record<NonNullable<OverlayView>, string>> = {
  ...
  // scope-reduction: hidden — coach: 'ai-coach',
  // scope-reduction: hidden — profiler: 'strength-analysis',
  // scope-reduction: hidden — strength: 'strength-analysis',
  injury: 'injury-illness',
  illness: 'injury-illness',
  // scope-reduction: hidden — custom_exercise: 'custom-exercises',
  templates: 'session-templates',
  // scope-reduction: hidden — grip_strength: 'grip-tracking',
  // scope-reduction: hidden — recovery_coach: 'ai-coach',
  block_suggestion: 'block-suggestions',
  // scope-reduction: hidden — volume_map: 'advanced-analytics',
  // scope-reduction: hidden — periodization: 'advanced-analytics',
  // scope-reduction: hidden — overload: 'advanced-analytics',
  recovery: 'advanced-analytics',
  // scope-reduction: hidden — hr_zones: 'wearable-integration',
  // scope-reduction: hidden — cycle_tracking: 'advanced-analytics',
  // scope-reduction: hidden — fatigue: 'advanced-analytics',
  // scope-reduction: hidden — fight_camp: 'fight-camp-nutrition',
  grappling: 'competition-prep',
  // scope-reduction: hidden — circuit_builder: 'advanced-analytics',
  // scope-reduction: hidden — photo_progress: 'advanced-analytics',
  // scope-reduction: hidden — breathing: 'advanced-analytics',
  // scope-reduction: hidden — split_analyzer: 'advanced-analytics',
  // scope-reduction: hidden — conditioning: 'advanced-analytics',
  // scope-reduction: hidden — fighters_mind: 'advanced-analytics',
  // scope-reduction: hidden — training_journal: 'advanced-analytics',
};
...
export default function Dashboard({
  ...
  // Full-screen overlay views — wrapped in overlay-safe for status bar clearance
  {
    ...
    const OVERLAY_COMPONENTS: Record<string, React.ReactNode> = {
      ...
      // scope-reduction: hidden — coach: <WeeklyCoach onClose={closeOverlay} />,
      // scope-reduction: hidden — profiler: <ExerciseProfiler onClose={closeOverlay} />,
      // scope-reduction: hidden — strength: <StrengthAnalysis onClose={closeOverlay} />,
      // scope-reduction: hidden — periodization: <PeriodizationCalendar onClose={closeOverlay} />,
      recovery: <RecoveryDashboard onClose={closeOverlay} />,
      injury: <InjuryLogger onClose={closeOverlay} />,
      // scope-reduction: hidden — overload: <ProgressiveOverload onClose={closeOverlay} />,
      // scope-reduction: hidden — custom_exercise: <CustomExerciseCreator onClose={closeOverlay} />,
      one_rm: <OneRepMaxCalc onClose={closeOverlay} />,
      // scope-reduction: hidden — hr_zones: <HRZoneTraining onClose={closeOverlay} />,
      templates: <SessionTemplates onClose={closeOverlay} />,
      // scope-reduction: hidden — volume_map: <VolumeHeatMap onClose={closeOverlay} />,
      grappling: <GrapplingTracker onClose={closeOverlay} />,
      // scope-reduction: hidden — community_share: <CommunityShare onClose={closeOverlay} />,
      quick_actions: <QuickActions onClose={closeOverlay} />,
      // scope-reduction: hidden — grip_strength: <GripStrengthModule onClose={closeOverlay} />,
      // scope-reduction: hidden — recovery_coach: <RecoveryCoach onClose={closeOverlay} />,
      block_suggestion: <BlockSuggestionView onClose={closeOverlay} />,
      // scope-reduction: hidden — user_guide: <NewUserGuide onComplete={closeOverlay} />,
      illness: <IllnessLogger onClose={closeOverlay} />,
      // scope-reduction: hidden — cycle_tracking: <CycleTracking onClose={closeOverlay} />,
      // scope-reduction: hidden — fatigue: <FatigueOverlay onClose={closeOverlay} />,
      // scope-reduction: hidden — fight_camp: <FightCampNutrition onClose={closeOverlay} />,
      // scope-reduction: hidden — badge_showcase: <BadgeShowcase onClose={closeOverlay} />,
      // scope-reduction: hidden — wellness_xp: <WellnessXPDashboardOverlay onClose={closeOverlay} />,
      // scope-reduction: hidden — auto_throttle: <AutoThrottleInfo onClose={closeOverlay} />,
      // scope-reduction: hidden — corner_coach: <CornerCoachInfo onClose={closeOverlay} />,
      // scope-reduction: hidden — training_load: <TrainingLoadDashboard onClose={closeOverlay} />,
      // scope-reduction: hidden — warm_up: <WarmUpInfo onClose={closeOverlay} />,
      plate_calc: <PlateCalculator onClose={closeOverlay} />,
      // scope-reduction: hidden — circuit_builder: <CircuitBuilder onClose={closeOverlay} />,
      // scope-reduction: hidden — photo_progress: <PhotoProgress onClose={closeOverlay} />,
      // scope-reduction: hidden — breathing: <BreathingProtocols onClose={closeOverlay} />,
      // scope-reduction: hidden — split_analyzer: <SplitAnalyzer onClose={closeOverlay} />,
      // scope-reduction: hidden — movement_library: <MovementLibrary onClose={closeOverlay} />,
      // scope-reduction: hidden — conditioning: <ConditioningSession onClose={closeOverlay} />,
      // scope-reduction: hidden — fighters_mind: <FightersMind onClose={closeOverlay} />,
      // scope-reduction: hidden — training_journal: <TrainingJournal onClose={closeOverlay} />,
      // scope-reduction: hidden — knowledge_hub: <KnowledgeHub onClose={closeOverlay} initialCategory={overlayContext as ContentCategory | undefined} />,
