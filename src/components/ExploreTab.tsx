          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {category.tools.map((tool, toolIdx) => (
              <motion.button
                ...
                className={cn(
                  'flex flex-col items-center justify-center gap-1.5 p-4 sm:p-3 rounded-xl bg-gradient-to-b border border-grappler-800/50 min-h-[5.5rem]',
                  'hover:border-grappler-700 active:scale-95 transition-all',
                  tool.color
                )}
              >
                <tool.icon className="w-5 h-5" />
                <span className="text-xs font-medium text-grappler-200 text-center leading-snug line-clamp-2">
                  {tool.label}
                </span>
              </motion.button>
            ))}
