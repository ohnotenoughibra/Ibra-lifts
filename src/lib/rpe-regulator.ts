...
import { rpeToPercentage, roundToIncrement } from './load-model';

// The RPE × reps → %1RM chart now lives in load-model.ts so the between-session
// suggestion and this within-session regulator agree on the same curve.
...
export function regulateRPE(
  ...
  const roundTo = (w: number) => roundToIncrement(w, weightUnit);
