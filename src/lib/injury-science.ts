...
export const REGION_AVOID_EXERCISES: Record<BodyRegion, string[]> = {
  ...
  chest:           ['bench-press', 'dumbbell-press', 'incline-bench', 'dips', 'push-up', 'cable-fly'],
  upper_back:      ['pendlay-row', 'barbell-row', 'pull-up', 'lat-pulldown', 'deadlift'],
  lower_back:      ['deadlift', 'romanian-deadlift', 'good-morning', 'pendlay-row', 'back-squat', 'barbell-row'],
...
const REGION_MODIFIED_EXERCISES: Record<BodyRegion, { exerciseId: string; modification: string }[]> = {
  ...
  lower_back:      [{ exerciseId: 'back-squat', modification: 'Switch to goblet squat or leg press' }, { exerciseId: 'barbell-row', modification: 'Use chest-supported row instead' }],
