...
const GRAPPLING_PRESETS: { name: string; description: string; build: () => WorkoutSession }[] = [
  ...
  {
    name: 'Rootsler Conditioning',
    description: 'High-rep compound work to build mat endurance',
    build: () => buildPresetSession('Rootsler Conditioning', 'hypertrophy',
  ...
  {
    name: 'Home Mat Work',
    description: 'Bodyweight strength for Rootslers — no equipment needed',
  ...
  },
  {
    ...
    description: 'Bulletproof your neck and grip — essential injury prevention for Rootslers',
  ...
  {
    name: 'Rootsler Power Circuit',
    description: 'Explosive rotational power & hip drive for takedowns and sweeps',
    build: () => buildPresetSession('Rootsler Power Circuit', 'power',
      ['hang-clean', 'med-ball-rotational-throw', 'kettlebell-swing', 'box-jump', 'turkish-getup', 'pallof-press'],
      3, 6, 8, 120),
  },
  {
    name: 'Striker Strength',
    description: 'Rotational power, shoulder endurance, and leg drive for boxing and Muay Thai',
    build: () => buildPresetSession('Striker Strength', 'power',
      ['push-press', 'landmine-press', 'med-ball-rotational-throw', 'box-jump', 'back-squat', 'face-pull'],
      4, 5, 8, 120),
  },
  {
    name: 'Wrestling Base',
  ...
  {
    name: 'Guard Player',
    description: 'Hip mobility, core compression, and pulling strength for guard retention and sweeps',
    build: () => buildPresetSession('Guard Player', 'hypertrophy',
      ['hip-thrust', 'hanging-leg-raise', 'pull-up', 'cable-row', 'pallof-press', 'copenhagen-plank'],
      3, 10, 7, 90),
  },
];
...
const QUICK_PRESETS: { name: string; description: string; category: PresetCategory; build: () => WorkoutSession }[] = [
  ...
  },
  {
    ...
    category: 'push',
    description: 'All three delt heads with overhead pressing — front, lateral, and rear',
    build: () => buildPresetSession('Shoulder Builder', 'hypertrophy',
      ['overhead-press', 'dumbbell-shoulder-press', 'lateral-raise', 'rear-delt-fly', 'face-pull', 'cable-lateral-raise'],
      4, 12, 7, 90),
  },
  {
  ...
  },
  {
    name: 'Core Fortress',
    category: 'full_body',
    description: 'Anti-extension, anti-rotation, and flexion — bulletproof your midsection',
    build: () => buildPresetSession('Core Fortress', 'hypertrophy',
      ['hanging-leg-raise', 'ab-wheel-rollout', 'pallof-press', 'plank', 'cable-woodchop', 'dead-bug'],
      3, 12, 7, 60),
  },
  {
    name: 'Power Day',
  ...
  },
  {
    ...
    category: 'push',
    description: 'Main bench + variations + tricep work. For pressing strength and chest mass',
    build: () => buildPresetSession('Bench Press Focus', 'strength',
      ['bench-press', 'incline-bench-press', 'close-grip-bench', 'dumbbell-press', 'skull-crusher', 'tricep-pushdown'],
      4, 5, 8, 180),
  },
  {
    name: 'Deadlift Focus',
    category: 'lower',
    description: 'Main deadlift + accessories for grip, back, and hamstrings',
    build: () => buildPresetSession('Deadlift Focus', 'strength',
      ['deadlift', 'barbell-row', 'romanian-deadlift', 'pull-up', 'farmers-walk', 'face-pull'],
      4, 5, 8, 180),
  },
  {
    name: 'Pump & Burn',
    category: 'full_body',
    description: 'High-rep, short-rest metabolic session. Chase the pump across every muscle group',
    build: () => buildPresetSession('Pump & Burn', 'hypertrophy',
      ['dumbbell-press', 'cable-row', 'lateral-raise', 'leg-press', 'bicep-curl', 'tricep-pushdown', 'calf-raise'],
      3, 15, 6, 45),
  },
];
...
const PROGRAM_TEMPLATES: ProgramTemplate[] = [
  ...
  },
  {
    ...
    name: "Rootsler's Edge",
    description: 'Balanced strength, power, and grip work. Designed around a training schedule with recovery in mind.',
    science: 'James et al. (2016): Concurrent strength and mat training requires careful volume management — 3-4 sessions/week is optimal for martial artists.',
    goalFocus: 'balanced',
    sessionsPerWeek: [3, 4],
    weeks: [4, 5, 6],
    periodization: 'undulating',
...
export default function SessionTemplates({ onClose }: SessionTemplatesProps) {
  ...
  return (
    <div className="min-h-screen bg-grappler-950 text-grappler-50 pb-24 safe-area-top">
      {/* Header */}
