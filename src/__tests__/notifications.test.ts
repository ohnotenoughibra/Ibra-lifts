...
describe('checkDailyLoginBonus', () => {
  // Use the SAME local-day key the function uses (checkDailyLoginBonus →
  // localDayKey/parseLocalDate). The old toISOString() UTC strings drifted a
  // day vs the function's local calendar day whenever local time and UTC fell
  // on different days (e.g. evening in the Americas), failing intermittently.
  ...
  it('resets streak after 2+ day gap', () => {
    ...
    const threeStr = localDayKey(threeDaysAgo);
  ...
  it('two-day gap resets even if previous streak was long', () => {
    ...
    const twoStr = localDayKey(twoDaysAgo);
